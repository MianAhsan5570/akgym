...
if($_POST) {	
	...
	$purchase_ID =  $_POST['orderId'];

	$orderDate 						= date('Y-m-d', strtotime($_POST['purchase_date']));
  ...
  $fetchthistransaction = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM purchase WHERE purchase_id = '$orderId'"));
	...
	if ($dueValue>0) {
		  	# code...
		// mysqli_query($dbc,"DELETE FROM transactions WHERE transaction_id = '$fetchTransaction[transaction_id]' ");
		 $fetchTransaction = mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM transactions WHERE customer_id='$clientName' ORDER BY transaction_id DESC LIMIT 1"));
		  	$credit =$dueValue;
		  	...
		  	$q= mysqli_query($dbc, "UPDATE transactions SET credit = '$credit' , debit = '0' WHERE transaction_id = '$fetchthistransaction[transaction_id]'");
		...
		if ($dueValue<0) {
		  	...
		  	$q= mysqli_query($dbc, "UPDATE transactions SET debit = '$debit', credit = '0' WHERE transaction_id = '$fetchthistransaction[transaction_id]'");
		  	$transaction_last_id = mysqli_insert_id($dbc);
		  	 mysqli_query($dbc,"UPDATE purchase SET transaction_id='$transaction_last_id' WHERE purchase_id='$order_id'");
		  }
		 if ($dueValue==0) {
