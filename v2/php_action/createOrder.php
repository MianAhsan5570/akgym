   ...
   //$fetchCustomer =mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM customers WHERE customer_id='$clientName'"));
	...
	if($connect->query($sql) === true) {
		...
		if ($dueValue>0) {
		  	...
		  	$q=mysqli_query($dbc,"INSERT INTO transactions(debit,credit,balance,customer_id,transaction_remarks,transaction_date) VALUES('0','$credit','$balance','$clientName','$transaction_remarks($clientContact)' , '$orderDate ')");
		...
		if ($dueValue<0) {
		  	...
		  	$q=mysqli_query($dbc,"INSERT INTO transactions(debit,credit,balance,customer_id,transaction_remarks,'transaction_date') VALUES('$debit','0','$balance','$clientName','$transaction_remarks','$orderDate ')");
		  	$transaction_last_id = mysqli_insert_id($dbc);
		  	 mysqli_query($dbc,"UPDATE orders SET transaction_id='$transaction_last_id' WHERE order_id='$order_id'");
		  }
