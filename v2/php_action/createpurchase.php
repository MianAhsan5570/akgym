	...
	if(mysqli_query($dbc,$sql)) {
		...
		if ($dueValue>0) {
			$debit =$dueValue;
		  	...
		  	$q=mysqli_query($dbc,"INSERT INTO transactions(debit,credit,balance,customer_id,transaction_remarks,transaction_date) VALUES('$debit','0','$balance','$clientName','$transaction_remarks','$purchaseDate')");
		...
		if ($dueValue<0) {
		  	...
		  	$q=mysqli_query($dbc,"INSERT INTO transactions(debit,credit,balance,customer_id,transaction_remarks,transaction_date) VALUES('0','$credit','$balance','$clientName','$transaction_remarks','$purchaseDate')");
		  	$transaction_last_id = mysqli_insert_id($dbc);
		  	 mysqli_query($dbc,"UPDATE purchase SET transaction_id='$transaction_last_id' WHERE purchase_id='$order_id'");
		  	# code...
