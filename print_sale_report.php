...
<div class="panel panel-danger">
	<div class="panel-heading" align="center"><h4>Sale Report</h4></div>
		...
		<table class="table">
			...
			<tbody>
				...
				if($f_date > 0 AND $t_date>0 AND $customer_id>0  ){
				 $q = mysqli_query($dbc,"SELECT * FROM orders WHERE (order_date BETWEEN '$f_date' AND '$t_date') AND client_name='$customer_id'");
}elseif($f_date > 0 AND $t_date>0){
	 $q = mysqli_query($dbc,"SELECT * FROM orders WHERE (order_date BETWEEN '$f_date' AND '$t_date')");
}else{
	 $q = mysqli_query($dbc,"SELECT * FROM orders WHERE  client_name='$customer_id'");
}
				 ?>
