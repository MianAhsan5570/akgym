...
echo "<div class='alert alert-{$sts}'>{$msg}</div>";
  ...
  function deleteFromTable($dbc,$table,$id,$fld=""){
  global $sts;
  global $msg;
  $id = base64_decode($id);
...
<!-- Email Function -->
  ...
  $mail = new PHPMailer;
  $headers = 'From: <info@onlinecashflowloans.com.au">' . "\r\n"; 
  //$mail->SMTPDebug = 2;
  $mail->isSMTP();            
  //Set SMTP host name                          
  ...
  $mail->Username = "info@onlinecashflowloans.com.au";                 
  $mail->Password = "Magnolia1";                           
  //If SMTP requires TLS encryption then set it
  $mail->SMTPSecure = "tls";                           
  //Set TCP port to connect to 
  $mail->Port = 587;                                   
//'leads@tornadofinance.com.au','info@magnoliafinance.com.au',
  $mail->From = "info@onlinecashflowloans.com.au";
  $mail->FromName = "Online Cashflow Loans";
  $mail->Body = $email_body;
  $mail->Subject = "Online Cashflow Loans Email Alerts"; 
  $allemails = array($email_address,'moixx.ansari43@gmail.com');

  foreach($allemails as $allemail){
    $mail->AddBCC($allemail);
  }
  $mail->AddBCC('moixx.ansari43@gmail.com');
  $mail->isHTML(true);
...
<?php 
 ...
 ?>
 <?php 
  function getQRURL($data,$size,$padding){
    ?>
 ...
 ?>
 <?php 
  function getQRTEL($number,$size,$padding){
    ?>
 ...
 ?>
  <?php 
  function getQREmail($email,$size,$padding){
    ?>
 ...
 ?>
  <?php 
  function getQRSMSTO($number,$msg,$size,$padding){
    ?>
 ...
 ?>
  ...
  <?php 
  function getChecked($data,$value){
    echo ($data==$value)?"checked":"";
  }
  ?>


<?php 
  function getInterviewerDetails($dbc,$id){
   $arr= mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM main_form WHERE id='$id'"));
    ?>
       <p><b>Name: </b><?=$arr['interviewer_name']?></p>
        <p><b>Phone: </b><?=$arr['interviewer_phone']?></p>
        <p><b>Email: </b><?=$arr['interviewer_email']?></p>
        <p><b>Interview Date: </b><?=date('D, d-M-Y',strtotime($arr['interview_date']))?></p>
        <p><b>UC: </b><?=$arr['union_concil_no']?></p>
        <p><b>Questionarie #: </b><?=$arr['questionarie_no']?></p>
    <?php
  }
 ?>

<?php 
  function getFarmerDetails($dbc,$id){
   $arr= mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM main_form WHERE id='$id'"));
    ?>
       <p><b>Name: </b><?=$arr['farmer_name']?></p>
        <p><b>Phone: </b><?=$arr['farmer_phone']?></p>
        <p><b>Email: </b><?=$arr['farmer_email']?></p>
        <p><b>Gender: </b><?=$arr['gender']?></p>
        <p><b>Age: </b><?=$arr['farmer_age']?></p>
        <p><b>Village: </b><?=$arr['village_name']?></p>
        <p><b>District:  </b><?=$arr['district']?></p>
        <p><b>Tehsil:  </b><?=$arr['district']?></p>
    <?php
  }
 ?>
