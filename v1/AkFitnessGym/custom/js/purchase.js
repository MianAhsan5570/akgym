		...
		if(productId == "") {
			...
			$.ajax({
				...
				success:function(response) {
					...
					$("#purchase_value"+row).val(response.purchase);
					$("#quantity_show"+row).val(response.quantity);
					$("#quantity"+row).val(1);
					

					var total = Number(response.purchase) * $("#quantity"+row).val();
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				...
				'<td style="padding-left:20px;"">'+
					...
					'<div class="input-group"><span class="input-group-addon">Stock</span><input type="text" readonly name="stock[]" id="quantity_show'+count+'" autocomplete="off" class="form-control" /></div>'+
				'</td style="padding-left:20px;">'+
				'<td style="padding-left:20px;">'+
					'<div class="form-group">'+
